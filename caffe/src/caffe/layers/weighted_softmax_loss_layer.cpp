...
void WeightedSoftmaxWithLossLayer<Dtype>::LayerSetUp(
  ...
  neg_mult_ = this->layer_param_.softmax_param().neg_mult();
  neg_cid_ = this->layer_param_.softmax_param().neg_cid();

  LOG(INFO) << "foreground_mult: " << pos_mult_ << ", foreground_id: " << pos_cid_<< "background_mult: " << neg_mult_ << ", background_id: " << neg_cid_;
...
void WeightedSoftmaxWithLossLayer<Dtype>::Forward_cpu(
  ...
  const Dtype* prob_data = prob_.cpu_data(); // get the -inf -> +inf probability of being fore and back 
  ...
  const Dtype* mask = bottom[2]->cpu_data();
   ...
   for (int i = 0; i < outer_num_; ++i) {
      ...
      const int mask_value = static_cast<int>(mask[i * inner_num_ + j]);
      ...
      loss -= mask_value * w * log(std::max(prob_data[i * dim + label_value * inner_num_ + j],Dtype(FLT_MIN)));
      ++count;
    }
  }
  if (normalize_) {
    ...
    for (int i = 0; i < outer_num_; ++i) {
      for (int j = 0; j < inner_num_; ++j) {
        ...
        if (has_ignore_label_ && label_value == ignore_label_) {
          ...
          Dtype w = (label_value == pos_cid_) ? pos_mult_ : neg_mult_;
          for (int k = 0; k < dim; ++k) {
            bottom_diff[i * dim + k * inner_num_ + j] *= w*mask_value;
          }
          ++count;
        }
      }
